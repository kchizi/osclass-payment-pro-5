...
s_extra VARCHAR( 250 ) NULL ,PRIMARY KEY(fk_s_pending_code),INDEX idx_fk_s_pending_code (fk_s_pending_code)) ENGINE=InnoDB DEFAULT CHARACTER SET 'UTF8' COLLATE 'UTF8_GENERAL_CI';", DB_TABLE_PREFIX));
       ...
       public function status($status) {
        ...
        public function invoices($params) {
            ...
            $tx = (isset($params['tx']) && $params['tx']!='') ? $params['tx'] : '';
            $this->dao->select('*') ;
            $this->dao->from($this->getTable_invoice());
            ...
            $invoices = $result->resultArray();
            $invIds = array_column($invoices, 'pk_i_id');
            ...
            if($result) {
                foreach ($invoices as $k => $inv) {
                    $invoices[$k]['rows'] = array();
                    foreach($result->resultArray() as $row) {
                        if ($row['fk_i_invoice_id'] == $inv['pk_i_id']) {
                            $invoices[$k]['rows'][] = $row;
                        }
                    }
                }
        ...
        public function setInvoiceExtra($invoiceId, $extra, $source = 'NOSOURCE') {
            ...
            if(isset($data['s_extra'])) {
                return $this->dao->update($this->getTable_invoice_extra(), array('s_extra' => $extra, 'fk_s_pending_code' => $invoiceId, 's_source' => $source), array('fk_i_invoice_id' => $invoiceId));
            } else {
                return $this->dao->insert($this->getTable_invoice_extra(), array('s_extra' => $extra, 'fk_s_pending_code' => $invoiceId, 'fk_i_invoice_id' => $invoiceId, 's_source' => $source));
            }
        }
