...
<?php if(count($packs)>0) {
    foreach($packs as $pack) { ?>
        <div class="payments-item" style="display:inline-block; float:left; border: 1px solid ; padding:5px; margin-right: 10px; margin-bottom:10px;">
            <h4 style="text-align:center;"><?php printf(__('Credit pack #%s', 'payment_pro'), str_pad($pack['pk_i_id'], 3, "0",STR_PAD_LEFT)); ?></h4>
            <h5><?php echo $pack['s_name']; ?></h5>
            <div style="text-align:center;"><label><?php printf(__("Price: %s", 'payment_pro'), osc_format_price($pack['i_amount_cost'], osc_get_preference('currency', 'payment_pro'))); ?></label></div>
            <div style="text-align:center;"><label><?php printf(__("Amount rewarded: %s", 'payment_pro'), osc_format_price($pack['i_amount'], payment_pro_currency())); ?></label></div>
            <ul class="payments-ul wallet-ul" style="text-align:center; padding-left:0px;">
