...
if(isset($payment_pro_publish_fee['price']) && $payment_pro_publish_fee['price']>0) {
    $category = Category::newInstance()->findByPrimaryKey($category_id, $locale);
$opt = '<div class="payments-box">' .
        '<div class="payments-box-header">' . __('Publish fee', 'payment_pro') . '</div>' .
        '<div class="payments-box-footer">' .
            '<label><input type="checkbox" name="dummy_check" id="dummy_check" value="1" checked="checked" disabled="disabled" /> ' . sprintf(__('Publishing this ad costs %s for %d days', 'payment_pro'), osc_format_price($payment_pro_publish_fee['price']*1000000, payment_pro_currency()), $category['i_expiration_days']) . '</label>' .
        '</div>' .
    '</div>' .
    '</div>';
    $pay_opt["PUB"] = $opt;
