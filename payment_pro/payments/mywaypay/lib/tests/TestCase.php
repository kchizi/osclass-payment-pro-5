...
/**
 * Base class for mywaypay test cases, provides some utility methods for creating
 * objects.
 */
class TestCase extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $apiKey = getenv('mywaypay_API_KEY');
        if (!$apiKey) {
            $apiKey = self::API_KEY;
        }

        mywaypay::setApiKey($apiKey);
    }
    ...
    protected function mockRequest($method, $path, $params = array(), $return = array('id' => 'myId'))
    {
        ...
        $mock->expects($this->at($this->call++))
             ->method('request')
             ->with(strtolower($method), 'https://api.mywaypay.com' . $path, $this->anything(), $params, false)
    ...
    private function setUpMockRequest()
    {
        if (!$this->mock) {
            ...
            $this->mock = $this->getMock('\mywaypay\HttpClient\ClientInterface');
