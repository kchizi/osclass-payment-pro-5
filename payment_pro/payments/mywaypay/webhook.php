...
$mywaypay = MywaypayPayment::getEnvironment();
...
try {
    $event = \mywaypay\Event::retrieve($event_id);
    ...
    if (isset($event->data) && isset($event->data->object) && isset($event->data->object->customer)) {
        ...
        if (isset($customer->email)) {
            ...
            $customer_email = "noemail@mywaypay.com";;
        }
    } else {
        // CUSTOMER ID MISSING ?
        print_r("CUSTOMER ID");
...
} catch(\mywaypay\Error\Api $e) {
    die;
}

osc_run_hook('payment_pro_mywaypay_webhook', $event);
...
switch($type) {
    ...
    case 'invoice.payment_succeeded':
        ...
        $exists = ModelPaymentPro::newInstance()->getPaymentByCode($event->data->object->id, 'mywaypay', PAYMENT_PRO_COMPLETED);
        ...
        // but we retrieve the event from the mywaypay server, so it should be safe
        ...
        if(isset($event->data) && isset($event->data->object) && isset($event->data->object->subscription)) {
            ...
            foreach($productsdb as $p) {
                if($first_payment || $p['i_count']==1) {
                    ...
                    if ($first_payment) {
                        ...
                        osc_run_hook('payment_pro_mywaypay_first_subscription', $sub_id, $p, $event->data->object->id, $item);
                    }
                }
            }
            // RECREATE EXTRA DATA
            ...
            foreach($items as $k => $item) {
                if (!$first_payment) {
                    $subitemid = ModelPaymentPro::newInstance()->updateSubscriptionItem($sub_id, $item['id'], PAYMENT_PRO_COMPLETED, $item, $event->data->object->id, "mywaypay");
