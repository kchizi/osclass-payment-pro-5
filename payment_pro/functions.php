<?php
    ...
    function payment_pro_send_email($email) {
        ...
        $emailParams =  osc_apply_filter('alert_email_payment_pro_params', array('subject'  => $title
        ...
        $data = "To:".$item['s_contact_email'].", ".$item['s_contact_name'].", ".$title;
        Log::newInstance()->insertLog('payment-pro', 'email', $item['pk_i_id'], $data, 'admin', osc_logged_user_id());
    ...
    function payment_pro_send_bank_notification($payment) {
        ...
        $emailParams =  osc_apply_filter('email_payment_pro_params', array('subject'  => $title
        ,'to'       => $user['s_email']
        ,'to_name'  => $user['s_name']
        ,'body'     => $body
        ,'alt_body' => $body));
    ...
    function payment_pro_tx_link($code, $source) {
        ...
        switch($source) {
            case 'BANK':
                $url = osc_route_admin_url('payment-pro-admin-bank-item', array('tx'=>$code));
                return sprintf($template, $url, $code);
            case 'PAYPAL':
                $url = 'https://www.paypal.com/uk/cgi-bin/webscr?cmd=_view-a-trans&id=' . $code;
                return sprintf($template, $url, $code);
            case 'STRIPE':
            case osc_get_preference('stripe_sca_source', 'payment_pro'):
