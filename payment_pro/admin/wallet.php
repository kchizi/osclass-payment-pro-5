    ...
    if(Params::getParam('plugin_action')=='done') {
        ...
        if(Params::getParam('userId')!='') {
            ...
            $invoiceId = ModelPaymentPro::newInstance()->saveInvoice(
                $user['pk_i_id'] . '-' . osc_genRandomPassword(),
                Params::getParam('amount'),
                0,
                Params::getParam('amount'),
                PAYMENT_PRO_COMPLETED,
                strtoupper(osc_get_preference('currency', 'payment_pro')),
                $user['s_email'],
                $user['pk_i_id'],
                'ADMIN',
                null
        ...
        <form name="payment_pro_form" action="<?php echo osc_admin_base_url(true); ?>" method="post">
            ...
            <fieldset>
                <div class="form-horizontal">
                    <div class="form-row">
                        <div class="form-label"><?php _e('Start typing to search for user', 'payment_pro'); ?></div>
                        <div class="form-controls"><input type="text" class="xlarge" id="user" name="user" value="" />
                        <span class="help-box">
                            <?php _e('by name or email', 'payment_pro'); ?>
                        </span>
                        </div>
                    </div>
                    <div class="form-row">
