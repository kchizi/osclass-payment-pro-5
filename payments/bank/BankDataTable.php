...
class BankDataTable extends DataTable
{
    ...
    public function table($params)
    {
        ...
        $data = array(
            ...
            'tx'  => (isset($params['tx']) && $params['tx']!='')?$params['tx']:Params::getParam('tx'),
            'source' => 'BANK'
        );
    ...
    private function processData($invoices)
    {
        if(!empty($invoices)) {
            ...
            foreach($invoices as $aRow) {
                ...
                $row['items'] = $this->_invoiceRows($aRow['rows'], $aRow['s_currency_code']);
                if($aRow['s_currency_code']=="BTC") {
                    ...
                    $row['subtotal'] = payment_pro_format_price($aRow['i_amount'], $aRow['s_currency_code']);
                    $row['tax'] = payment_pro_format_price($aRow['i_amount_tax'], $aRow['s_currency_code']);
                    $row['total'] = payment_pro_format_price($aRow['i_amount_total'], $aRow['s_currency_code']);
                }
                ...
                $row['delete'] = '<a onclick="javascript:return confirm(\'' . osc_esc_html(__('Are you sure you want to delete this invoice?', 'payment_pro')) . '\');" href="' . osc_route_admin_url('payment-pro-admin-bank', array('paction' => 'delete', 'id' => $aRow['pk_i_id'])) . '" title="'. __('delete', 'payment_pro') . '"><i class="text-danger bi bi-x-circle-fill"></i></a>';
    ...
    private function _invoiceRows($items, $currency) {
        ...
        foreach($items as $item) {
            $rows[] = payment_pro_format_price($item['i_amount'], $currency) . ' - ' . $item['i_product_type'] . ' - ' . $item['s_concept'];
        }
