...
class ApiRequestor
{
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = mywaypay::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"mywaypay::setApiKey(<API-KEY>)".  You can generate API keys from '
              . 'the mywaypay web interface.  See https://mywaypay.com/api for '
              . 'details, or email support@mywaypay.com if you have any questions.';
        ...
        $uname = php_uname();
        $ua = array(
            'bindings_version' => mywaypay::VERSION,
            'lang' => 'php',
        ...
        $defaultHeaders = array(
            'X-mywaypay-Client-User-Agent' => json_encode($ua),
            'User-Agent' => 'mywaypay/v1 PhpBindings/' . mywaypay::VERSION,
            'Authorization' => 'Bearer ' . $myApiKey,
        );
        if (mywaypay::$apiVersion) {
            $defaultHeaders['mywaypay-Version'] = mywaypay::$apiVersion;
        }
        $hasFile = false;
